...
# --- *** MODIFICACIÓN 1 *** ---
# Añadida la lógica para los clientes de la Línea 1
def generar_clientes_precargados(linea_id):
    global id_articulo_global
    
    if linea_id == '1':
        clientes = [
            { "id": id_articulo_global + 40, "nombre_cliente": "Covirán Madrid", "articulos": [],
              "act_hora_entrada": False, "hora_entrada": datetime.time(8, 0) },
            { "id": id_articulo_global + 50, "nombre_cliente": "Covirán Granada", "articulos": [],
              "act_hora_entrada": False, "hora_entrada": datetime.time(8, 0) }
        ]
        return clientes

    # Lógica existente para las otras líneas
    clientes = [
...
# --- *** MODIFICACIÓN 2 *** ---
# Añadida la Línea 1 al bucle de inicialización
for linea_id in ['5', '4', '3', '1']:
# --- *** FIN DE LA MODIFICACIÓN *** ---
...
with tab_cfg:
    ...
    # --- *** MODIFICACIÓN 3 *** ---
    # Dibujar la configuración de la Línea 1
...
with tab_sim:
    ...
    if st.button("Iniciar Simulación Global", key="start_sim_button_global", type="primary"):
        # --- *** MODIFICACIÓN 4 *** ---
        # Añadir Línea 1 al setup de la simulación
        ...
        sims = [sim_l5, sim_l4, sim_l3, sim_l1]
        # --- *** FIN DE LA MODIFICACIÓN *** ---
        ...
        if not active_sims:
            ...
            while any(s['active'] for s in sims):
                ...
                # --- *** MODIFICACIÓN 5 *** ---
                # Procesar el "tick" de la Línea 1 y añadirla a los totales
                ...
                with placeholder_metricas_lineas.container():
                    ...
                    # --- *** MODIFICACIÓN 6 *** ---
                    # Cambiar a 4 columnas y añadir Línea 1
                ...
                with placeholder_graficos_lineas.container():
                    ...
                    def show_graph(sim_state):
                        ...
                        if not sim_state['historial_produccion']:
                            # Adaptamos el mensaje para líneas sin pedidos (como la L1 por defecto)
                    ...
                    show_graph(sim_l3)
                    # --- *** MODIFICACIÓN 7 *** ---
                    # Añadir el gráfico de la Línea 1
                    st.markdown("---")
                    show_graph(sim_l1)
                    # --- *** FIN DE LA MODIFICACIÓN *** ---
                ...
                with placeholder_progreso.container():
                    ...
                    # --- *** MODIFICACIÓN 8 *** ---
                    # Añadir la barra de progreso de la Línea 1
            ...
            # --- Tablas Finales (Sin cambios, el bucle "for s in sims:" se encarga) ---
